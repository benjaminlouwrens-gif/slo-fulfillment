...
(async () => {
  ...
  try {
    for (const [name, width, height] of [['desktop', 1440, 900], ['mobile', 390, 844]]) {
      ...
      await page.waitForFunction(() => document.querySelector('.surf-transition').dataset.transition === 'complete');
      assert.ok(Date.now() - forwardStartedAt < 1800, 'forward wave snap took too long');
      ...
      assert.ok(Math.abs(completeY - height * 2) <= 2, 'forward wave did not land on the story');
      ...
      await page.mouse.wheel(0, -160);
      await page.waitForFunction(() => document.querySelector('.surf-transition').dataset.transition === 'idle');
      assert.ok(Date.now() - reverseStartedAt < 1800, 'reverse wave snap took too long');
