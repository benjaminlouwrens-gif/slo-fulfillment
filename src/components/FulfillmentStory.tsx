...
function Product({ name, className = '' }: { name: string; className?: string }) {
  return <span className={`sl-product sl-product-${name} ${className}`} role="img" aria-label={name === 'fruit' ? 'Colorful cut fruit' : name} />;
}
const services = [
...
export function FulfillmentStory() {
  return <div className="sl-story">
    <div className="sl-route"><span>Good brands. Great partners.</span><span>Southern California to everywhere <ArrowUpRight size={18} /></span></div>
    ...
    <section className="sl-services sl-band" id="services">
      ...
      <div className="sl-service-layout"><div className="sl-product-duet"><Product name="skincare" /><Product name="croissant" /><p>Good taste.<br />Great care.</p></div><div className="sl-services-list">{services.map(([title, label, copy]) => <article key={title}><p className="sl-label">{label}</p><h3>{title}</h3><p>{copy}</p></article>)}</div></div>
    ...
    <section id="how-it-works" className="sl-process sl-band"><div className="sl-section-top"><h2>A little hello.<br />A big handoff.</h2><a className="sl-link" href="#contact">Let's get acquainted <ArrowUpRight /></a></div><ol>{[
    ...
    <section className="sl-local sl-band"><div className="sl-local-copy"><p>Rooted here. Ready to go places.</p><h2>SoCal is<br />our home turf.</h2><p>Find a fulfillment partner close to the ports, your customers, or the next stop in your supply chain.</p></div><div className="sl-destinations"><span>Los Angeles</span><span>Orange County</span><span>Inland Empire</span><span>Ontario & Riverside</span><span>San Diego</span><a href="#contact">Your next stop <ArrowUpRight /></a></div></section>
